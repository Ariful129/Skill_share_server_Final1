        ...
        if (status === 'success') {
            try {
                ...
                if (data && data.statusCode === '0000') {
                    ...
                    return res.redirect(`http://localhost:5173/courses/${Data.amount}?message=${Data.statusMessage}`)
                }else{
                    console.log("payment is undefined");
                    return res.redirect(`http://localhost:5173/courses/${Data.amount}?message=${Data.statusMessage}`)
                }
            } catch (error) {
                console.log("oyy", error,Data)
                if(Data.statusMessage=='Successful')
                return res.redirect(`http://localhost:5173/courses/${Data.amount}?message=${Data.statusMessage}`)
                else
                return res.redirect(`http://localhost:5173/courses/${Data.amount}?message=${error.message}`)
